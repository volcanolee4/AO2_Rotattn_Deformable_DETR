...
class RotatedDeformableDETRHead(RotatedDETRHead):
    ...
    def __init__(self,
        ...
        ########Add for angle_sigmoid#####
        self.angle_sigmoid = kwargs['angle_sigmoid']
        if self.angle_sigmoid:
            transformer['angle_sigmoid']=True
        else:
            transformer['angle_sigmoid'] = False

        ########Add for rotattn###########
        self.rotattn = kwargs['rotattn']
        if self.rotattn:
            transformer['rotattn']=True
        else:
            transformer['rotattn'] = False

        self.angle_factor_mat = [1., 1., 1., 1., math.pi / 2.]
        ###################################
    ...
    def forward(self, mlvl_feats, img_metas):
        ...
        for lvl in range(hs.shape[0]):
            ...
            #------------------------
            ############for angle rotattn################
            device,dtype = tmp.device , tmp.dtype
            scale_factor = torch.tensor(self.angle_factor_mat,device=device,dtype=dtype)
            outputs_coord = tmp.sigmoid() * scale_factor
            #########################################
        ...
        if self.as_two_stage:
            return outputs_classes, outputs_coords, \
                enc_outputs_class, \
                enc_outputs_coord.sigmoid() * scale_factor ###########
        else:
            return outputs_classes, outputs_coords, \
                None, None
