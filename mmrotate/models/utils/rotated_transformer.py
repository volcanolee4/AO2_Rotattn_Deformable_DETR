...
def my_obb2xywh_oc(rbboxes):
    ...
    w = rbboxes[..., 2]
    h = rbboxes[..., 3]
    a = rbboxes[..., 4] * math.pi / 2.
    cosa = torch.cos(a) #弧度制
    sina = torch.sin(a)
    ...
    return torch.stack((dx, dy, hbbox_w ,hbbox_h), -1)##w就是横边，h就是宽边，并不需要特殊辨认
...
class RotatedDeformableDetrTransformer(Transformer):
    ...
    def __init__(self,
        ...
        ##############For angle_sigmoid######################
        self.angle_sigmoid = angle_sigmoid
        ################For rotattn#####################
        self.rotattn =rotattn
        ################For rotattn#####################
        self.init_layers()
    ...
    def gen_encoder_output_proposals(self, memory, memory_padding_mask,
        ...
        for lvl, (H, W) in enumerate(spatial_shapes):
            ...
            #######C
            if self.angle_sigmoid:
                angle = torch.ones_like(mask_flatten_) * 1e-5
            else:
    ...
    def get_proposal_pos_embed(self,
        ...
        if proposals.shape[-1] == 8:
            proposals = proposals * scale #在这里面做了sigmoid
        else:
    ...
    def forward(self,
        ...
        if self.as_two_stage:
            ...
            topk_coords_unact = topk_coords_unact.detach()  #选出来后Detach了
            # obb2xyxy
            # reference_points = obb2poly_tr(topk_coords_unact).sigmoid() 它的方案是不断地在把参数以及四参数之间转换
            #######C
            if self.angle_sigmoid:
            ...
            if self.angle_sigmoid:
                pos_trans_out = self.pos_trans_norm(
                    self.pos_trans(
                        self.get_proposal_pos_embed(obb2poly_tr(reference_points), num_pos_feats=64)))  # 要不这里用外接矩形试试？
            else:
                pos_trans_out = self.pos_trans_norm(
...
class RotatedDeformableDetrTransformerDecoder(TransformerLayerSequence):
    ...
    def forward(self,
        ...
        rotattn = kwargs['rotattn']
        for lid, layer in enumerate(self.layers):
            if reference_points.shape[-1] == 4:
                reference_points_input = reference_points[:, :, None] * \
                                         torch.cat([valid_ratios, valid_ratios], -1)[:, None]
            if reference_points.shape[-1] == 5:
                if rotattn:       #reference_points[:, :, None]:[2，300，1，5]       second:[2,1,4,5]
                    device,dtype = valid_ratios.device , valid_ratios.dtype
                    angle_ratio = torch.ones(size=(reference_points.shape[0], 4, 1),device=device,dtype=dtype)  #[2，4，1]reference_points[2,300,5]
                    reference_points_input = reference_points[:, :, None] * \
                                             torch.cat([valid_ratios, valid_ratios,angle_ratio], -1)[:, None]  # 得到在四个特征图尺寸上的坐标和长宽
                else:
                    reference_points_hb_xywh = my_obb2xywh_oc(reference_points) #[x,y,w,h，a] -> 外接矩形：[x,y,w,h]
                    reference_points_input = reference_points_hb_xywh[:, :, None] * \
                                             torch.cat([valid_ratios, valid_ratios], -1)[:, None]  # 得到在四个特征图尺寸上的坐标和长宽
            else:
                assert reference_points.shape[-1] == 2
                reference_points_input = reference_points[:, :, None] * \
                                         valid_ratios[:, None]
            kwargs['rotattn'] = rotattn
            output = layer(
                output,
                *args,
